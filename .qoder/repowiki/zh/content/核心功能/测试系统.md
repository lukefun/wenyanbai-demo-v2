...
### 组件五：测验统计数据跟踪（新增）
- trackQuiz() 函数
  - 通过题目例句匹配学习卡片，为每个字符建立测验历史记录 qa（包含尝试次数 t 和正确次数 c）。
  - 自动维护每道题对应的字符统计，支持后续的详细统计分析。
- 统计详情弹窗
  - 提供三种统计视图：学过含义详情、测验正确率详情、测验次数详情、字已掌握详情。
  - 按字符维度展示正确率和测验次数，支持排序和筛选。

```mermaid
flowchart TD
Answer["答题完成"] --> Track["trackQuiz(q,correct)"]
Track --> Match["匹配题目例句到学习卡片"]
Match --> InitQA["初始化qa记录"]
InitQA --> UpdateT["qa.t++"]
UpdateT --> CheckCor{"是否正确？"}
CheckCor --> |是| UpdateC["qa.c++"]
CheckCor --> |否| Skip["不计分"]
UpdateC --> Save["保存到localStorage"]
Skip --> Save
Save --> Detail["统计详情弹窗"]
Detail --> Accuracy["测验正确率详情"]
Detail --> Count["测验次数详情"]
Detail --> Learned["学过含义详情"]
Detail --> Mastered["字已掌握详情"]
```

图表来源
- [app.js:303-316](file://app.js#L303-L316)
- [app.js:317-337](file://app.js#L317-L337)
- [app.js:366-400](file://app.js#L366-L400)
- [app.js:402-428](file://app.js#L402-L428)
...
## 结论
本系统通过清晰的模块划分与简洁的前端实现，提供了高效的文言文测试体验。正式测验与中途测验分别服务于系统性复习与即时巩固，配合随机抽题与选项生成逻辑，有效提升了学习效果与记忆强度。新增的 trackQuiz() 函数进一步完善了统计分析体系，为学习者提供了更细致的学习轨迹追踪和个性化反馈。建议在未来版本中扩展更多题型与难度分级，并增加错题回顾与学习路径规划功能。
...
### C. 新增功能详解
- trackQuiz() 函数实现原理
  - 通过去除 HTML 标签后的例句进行精确匹配，定位到对应的学习卡片。
  - 为每个字符建立 qa 统计对象，记录测验尝试次数与正确次数。
  - 支持后续的详细统计分析，包括按字符维度的正确率和测验次数统计。
- 统计详情弹窗功能
  - 提供四种统计视图：学过含义详情、测验正确率详情、测验次数详情、字已掌握详情。
  - 支持按正确率排序和按测验次数降序排列，帮助用户识别薄弱环节。
  - 字已掌握详情展示各字词的掌握进度和完成率，支持可视化进度条显示。
